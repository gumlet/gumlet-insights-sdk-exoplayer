...
class RetryQueue {
    ...
    fun addSample(retrySample: RetrySample<Any>) {
        try {
            ...
            if (retrySamplesList.size >= getMaxSampleNumber()) {
               ...
               GumletLog.d(TAG, "removed sample with highest scheduled time ${removeSample.scheduledTime} due to queue being over capacity of ${getMaxSampleNumber()}")
            }
            retrySamplesList.add(retrySample)
            retrySamplesList.sortWith(sampleComparator)
        } catch (e: Exception) {
            GumletLog.e(TAG, "addSample threw an unexpected exception: ${e.message}", e)
        } finally {
            lock.unlock()
        }
    }
    ...
    fun getNextSampleOrNull(): RetrySample<Any>? {
        try {
            ...
            GumletLog.e(TAG, "getSample threw an unexpected exception: ${e.message}", e)
        } finally {
            lock.unlock()
        }
        return null
    ...
    fun getNextScheduleTime(): Date? {
        try {
            ...
            GumletLog.e(TAG, "getNextScheduleTime threw an unexpected exception ${e.message}", e)
        }
        return null
    }
}
